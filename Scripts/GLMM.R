...
replicate <- rep(1:3, each = length(unique(data$Spcies)), times = 4)
data$replicate <- factor(replicate)

# Convert the data to long format for analysis
library(tidyr)
data_long <- pivot_longer(data, cols = -c(Spcies, treatment, replicate), names_to = "replicate_num", values_to = "response")
